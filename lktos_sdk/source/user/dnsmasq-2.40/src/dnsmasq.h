...
/* by luot for open windows moduel */
typedef struct dnsspoof_fctl_ipmac_entry
{
	unsigned char is_Hook;
	unsigned char idle_Count;
	unsigned char is_Online;
	unsigned char is_Auth_Ok;
	unsigned char is_Spoof;
       unsigned long sip;
	unsigned char smac[6];
}T_DNSSPOOF_FCTL_IPMAC_ENTRY;

typedef struct dnsspoof_ipmac_fctl_
{
	unsigned int is_Enable;
	unsigned long sub_ip;
	unsigned long lan_ip;
	unsigned int current_Num;
	T_DNSSPOOF_FCTL_IPMAC_ENTRY lan_ipmac_entry_tbl[255];
}T_DNSSPOOF_IPMAC_FCTL_, *T_DNSSPOOF_IPMAC_FCTL_p;
...
#define OPT_DNSSPOOF_MODE1	(1<<30)
#define OPT_DNSSPOOF_MODE2	(1<<31)
