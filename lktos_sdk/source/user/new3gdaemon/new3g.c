...
int write_str_ex(int fd, char *str, int len)
{
	...
	while(1)
	{
		...
		{
			cops += readlen;
			printf("\r\nin write_ex--%s",rddevbuf);
			if(strstr(rddevbuf,"OK"))
			{
				printf("\r\nin write_ex11--%s",rddevbuf);
...
int checkModuleSimpleStatus(int fd_mod,CDMAInfo *pcdma_csq_info, char *netcode)
{
...
/******************  AT+CIMI  START ***************************************************/
	...
	printf("\r\nsend at+cimi return--%s",rddevbuf);
	if(strstr(rddevbuf,"OK"))
	{
		...
		if(k != -1) //列表中有相同的对应关系
		{
			if(!strncmp(OptName[k].ID,"46000",5) || !strncmp(OptName[k].ID,"46002",5) || !strncmp(OptName[k].ID,"46007",5))
			{
				g_DT_modem=1;
			}
...
/******************  ATI  START ***************************************************/
	...
	printf("\r\n send at CGSN return--%s",rddevbuf);
...
int main(int argc, char **argv)
{
	...
	if(dial_style)
	{
		...
		{
			printf("3g dial type mismatch so exit!\n");
			exit(-1);
		}
	...
	while(1)
	{
		if(discon_flag)//断开拨号连接该变量一般有其它进程发送信号过来改变
		{
		...
		if ( ret == _DISCONNECT ) //如果是断开连接的,则拨号
		{
			...
			if(at_flag==0)
			{
				...
				{			
					...
					if(!dial_style)
					{
						ret = checkModuleSimpleStatus(fdd,&cdma_csq_info,NULL);
					}
					else
					{
					}
					printf("\r\n after check status gkard =%d",ret);
					if(ret < 0)
					{						
						close(fdd);
						fdd = 0;
						printf("check module error so continue\n");
						goto done;
					}
					
					system("echo 3434 > /var/trtrtr111");
					if(!dial_style) //AUTO
					{
			...
			else if(g_DT_modem)
			{
				...
				fprintf( fp, "OK\t\"ATS0=0\"\n");
				fprintf( fp, "OK\t'AT+CGDCONT=1,\"IP\",\"cmnet\"'\n");
				fprintf ( fp, "\"\"\t\"AT+CFUN=1\"\n");
